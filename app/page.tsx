...
function NegotiatorApp({ session }: { session: Session }) {
  ...
  useEffect(() => {
    if (isLoading) {
      const messages = [
        ...
        "Reading the seller's tone...",
        "Crafting negotiation scripts...",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const responseData: AiResponseType = result.data;
    ...
    <div className="w-full max-w-2xl">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-4">
          <Link href="/history" className="bg-gray-600 hover:bg-gray-500 text-white text-sm font-bold py-2 px-4 rounded-md">
            History
          </Link>
          <button onClick={handleSignOut} className="bg-red-600 hover:bg-red-700 text-white text-sm font-bold py-2 px-4 rounded-md transition-all">
            Sign Out
          </button>
        </div>
      </div>
            ...
            {aiResponse.scripts.map((script, index) => (
              <div key={index} className="bg-gray-700 p-4 rounded-md transition-all hover:bg-gray-600">
                <div className="flex justify-between items-center mb-2">
                  <h4 className="font-bold text-gray-200">{script.title}</h4>
                  <button onClick={() => { navigator.clipboard.writeText(script.content); setCopiedIndex(index); setTimeout(() => setCopiedIndex(null), 2000); }} className={`text-xs text-white font-semibold py-1 px-3 rounded-md transition-all active:scale-95 ${copiedIndex === index ? 'bg-green-600' : 'bg-gray-600 hover:bg-gray-500'}`}>
    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-4 bg-gray-900 text-white">
      {!session ? (
        <div className="w-full max-w-md p-6 bg-gray-800 rounded-lg shadow-2xl">
          <h1 className="text-3xl font-bold text-center mb-2">AI FairFare Negotiation </h1>
